    ...
    public static function execOptionMakeClearCache()
     {
         ...
         if ($cacheDir == false) {
             ...
             Debug::log('Um erro desconhecido ocorreu ao limpar o cache da aplicação');
             return;
         }
         if (count($cacheDir) > 2) {
             $clearCache = shell_exec('sudo rm -rf System/SysConfig/Cache/*');
             Cli::println('Diretório de Cache limpo com suscesso');
             Debug::log('Diretório de Cache limpo com suscesso');
         } else {
             Cli::println('Você não possui nenhum arquivo de cache disponivel para ser deletado.');
             Debug::log('Você não possui nenhum arquivo de cache disponivel para ser deletado.');
         }
     }
}
